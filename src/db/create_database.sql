...
INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('tokenizer', 1, 'preprocessing', 'tokenizer separa la frase en cada una de las palabras. Configuración: no configurable','{"code": "from pyspark.ml.feature import Tokenizer\nif (bool(pipeline_preprocessing_stages)):\n\tTOKENIZER = Tokenizer(inputCol=pipeline_preprocessing_stages[-1].getOutputCol(), outputCol=\"tokenizer_col\")\nelse:\n\tTOKENIZER = Tokenizer(inputCol=\"features\", outputCol=\"tokenizer_col\")\npipeline_preprocessing_stages.append(TOKENIZER)\n", "params":"[]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('hashingtf', 1, 'preprocessing', 'hashingtf es una transformación que convierte texto en un vector de variables que el modelo puede procesar. Configuración: no configurable.','{"code": "from pyspark.ml.feature import HashingTF\nif (bool(pipeline_preprocessing_stages)):\n\tHASHINGTF = HashingTF(inputCol=pipeline_preprocessing_stages[-1].getOutputCol(), outputCol=\"hashingtf_col\")\nelse:\n\tHASHINGTF = HashingTF(inputCol=\"features\", outputCol=\"hashingtf_col\")\npipeline_preprocessing_stages.append(HASHINGTF)\n", "params":"[]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('logistic_regression', 1, 'model', 'Spark logistic regression. Requiere que la etapa anterior al modelo sea hashingtf. Configuración: {"maxIter":<número máximo de iteraciones>, "regParam":<valor del parámetro de regularización>}. Los valores por defecto son los siguientes: maxIter=100, regParam=0.0.', '{"code": "from pyspark.ml.classification import LogisticRegression\nlr = LogisticRegression(maxIter=100, regParam=0.0)\npipeline_models_stages.append((lr.uid, lr))\n", "params":"[<lr_max_iter>,<lr_reg_param>]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('naive_bayes', 1, 'model', 'Spark naive_bayes. Configuración: {"smoothing":<valor del parámetro de smoothing>}. Los valores por defecto son los siguientes: smoothing=1.0.', '{"code": "from pyspark.ml.classification import NaiveBayes\nnb = NaiveBayes(smoothing=1.0, modelType=\"multinomial\")\npipeline_models_stages.append((nb.uid, nb))\n", "params":"[<smoothing>,<modelType>]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('linear_svc', 1, 'model', 'Spark Support Vector Machine. Configuración: {"maxIter":<número máximo de iteraciones>, "regParam":<valor del parámetro de regularización>}. Los valores por defecto son los siguientes: maxIter=100, regParam=0.0.', '{"code": "from from pyspark.ml.classification import LinearSVC\nlsvc = LinearSVC(maxIter=100, regParam=0.0)\npipeline_models_stages.append((lsvc.uid, lsvc))\n", "params":"[<maxIter>,<regParam>]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('decision_tree', 1, 'model', 'Spark decision tree. Configuración: {"maxDepth":<profundidad máxima del árbol>}. Los valores por defecto son los siguientes: maxDepth=10.', '{"code": "from from pyspark.ml.classification import DecisionTreeClassifier\ndt = DecisionTreeClassifier(maxDepth=10, labelCol=\"label\")\npipeline_models_stages.append((dt.uid, dt))\n", "params":"[<maxDepth>,<labelCol>]"}');
