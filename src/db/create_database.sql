...
INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('logistic_regression', 1, 'model', 'Spark logistic regression', '{"code": "from pyspark.ml.classification import LogisticRegression\nlr = LogisticRegression(maxIter=10, regParam=0.001)\npipeline_models_stages.append((lr.uid, lr))\n", "params":"[<lr_max_iter>,<lr_reg_param>]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('naive_bayes', 1, 'model', 'Spark naive_bayes.\n Configuración: {"smoothing":<valor del parámetro de smoothing>}', '{"code": "from pyspark.ml.classification import NaiveBayes\nnb = NaiveBayes(smoothing=1.0, modelType=\"multinomial\")\npipeline_models_stages.append((nb.uid, nb))\n", "params":"[<smoothing>,<modelType>]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('lineal_svm', 1, 'model', 'Spark Support Vector Machine.\n Configuración: {"maxIter":<número máximo de iteraciones>, "regParam":<valor del parámetro de regularización>}', '{"code": "from from pyspark.ml.classification import LinearSVC\nlsvc = LinearSVC(maxIter=10, regParam=0.1)\npipeline_models_stages.append((lsvc.uid, lsvc))\n", "params":"[<maxIter>,<regParam>]"}');

INSERT INTO code_block_templates(template_name, model_engine, type, description, code_content)
VALUES ('decision_tree', 1, 'model', 'Spark decision tree.\n Configuración: {\"maxDepth\":<profundidad máxima del árbol>}', '{"code": "from from pyspark.ml.classification import DecisionTreeClassifier\ndt = DecisionTreeClassifier(maxDepth=3, labelCol=\"label\")\npipeline_models_stages.append((dt.uid, dt))\n", "params":"[<maxDepth>,<labelCol>]"}');
