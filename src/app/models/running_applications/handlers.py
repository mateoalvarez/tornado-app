...
class RunningApplicationsHandler(BaseHandler):
    ...
    def _get_running_applications(self, db_cur, user):
        ...
        db_cur.execute(
            """SELECT id, application_name, application_status
            FROM applications
            WHERE user_id=%s AND application_status='running';""",
            (str(user["id"]),))
        applications = db_cur.fetchall()
        return applications
    ...
    def _put_application(self, db_cur, db_conn, user, application):
        ...
        db_cur.execute(
            ...
            (user["id"],
             application["training_config_resources"],
             ...
             application["datasource_configuration"],
             application["datasource_settings_id"]))
        try:
            db_conn.commit()
    ...
    def get(self):
        """GET method
        This method will have an parameter to discover in case it is GET
        running_application/{id_application}"""
    ...
    def get(self):
        ...
        if application_id == "":
            ...
            records_to_show = list(
                ...
                ["application_" + str(application_id)]
                .find().sort([("_id", -1)]).limit(int(last_elements)))
