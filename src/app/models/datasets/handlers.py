    ...
    def get(self):
        ...
        if user_datasets_s3["KeyCount"] > 0:
            for user_dataset_s3 in user_datasets_s3["Contents"]:
                ...
                db_user_datasets = self._get_dataset_from_database_by_s3_key(
                    self.current_user["id"], aux_user_dataset["Key"])
    ...
    def post(self):
        ...
        for field_name, files in self.request.files.items():
            for info in files:
                ...
                LOGGER.info('POST "%s" "%s" %d bytes', filename, content_type,
                            len(body))
    ...
    def post(self):
        ...
        if dataset_from_db is not None:
            ...
            s3_dataset_key = "{s3_bucket_prefix}/{s3_resource}".format(
                ...
                s3_resource=dataset_from_db["dataset_name"])
            self.S3_CLIENT.delete_object(
                Bucket=self.BUCKET_DATASETS,
                Key=s3_dataset_key
            )
        else:
            LOGGER.warning("""There is any dataset in database with id
                           {dataset_id} for user {user_id}""".format(
                               dataset_id=dataset_id_to_delete,
